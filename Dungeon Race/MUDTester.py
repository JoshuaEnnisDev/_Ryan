...
world = {
      'users':[],
      'store':[],
      'rooms':[{'id':1, 'desc': 'a large room', 'north':None, 'south':None, 'east':None, 'west':None}],
      'monsters': [],
      'items': []
  }
...
def http_response(headers, msg):
    return "HTTP/1.1 200 OK\r\n%s\r\n\r\n%s" % ('\r\n'.join(headers), html_page(css, msg))
                                                

def html_page(css, msg):
    return """
<!DOCTYPE html>
  <html>
    <head>
...
def new_user(username):   
    ...
    txt = yield f"Welcome {username.title()}!<br> Type 'load world' to begin."
...
def store(txt, user):
    ...
    while choice == None:
        ...
        if name == "q":
            yield "Back to the race!<br>" + next(look(txt,user))
            return
...
def move(txt, user):
    if txt in DIRECTIONS:
        ...
        if txt in room and room[txt] != None:
            for monster in filter(match_by('room_id', room['id']), world['monsters']):
                yield f"The {monster['name']} is blocking your path<br>" + next(look(txt,user))
                return
...
def attack(txt, user):
    ...
    if monster['dead'] == True:
        yield f"{name} is already defeated"
...
def use_item(txt, user):
    ...
    if 'health' in item:
        ...
        result = f"You cannot use the {item['name']}"
    yield result
